...
from data_import.data_import import get_data_points_list, image_generator
...
# Training image dataset (with label)
no_train_points = int(split_ratio[0] / sum(split_ratio) * dataset_len)
data_points_train = shuffled_data_points[:no_train_points]

data_gen_train = image_generator(data_points_train, no_epochs, no_classes, output_img_shape)
...
data_gen_val = image_generator(data_points_val, no_epochs, no_classes, output_img_shape)
...
data_gen_test = image_generator(data_points_test, no_epochs, no_classes, param_list)
...
model = LeNet.build(input_shape=output_img_shape,  classes=no_classes)
...
# Saving model
model.save('./results/trained_modelLeNet')

# Print and save on disk model training history
with open('./results/reportLeNet.json', 'w', encoding='utf-8') as f:
